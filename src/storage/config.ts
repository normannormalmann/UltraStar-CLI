...
export const loadConfig: Effect.Effect<AppConfig | null, Error> =
  Effect.gen(function* () {
    ...
    return yield* Effect.catchAll(
      Effect.tryPromise({
        ...
        catch: (e) => (e instanceof Error ? e : new Error("Failed to read config")),
      }),
      () => Effect.succeed(null),
    );
  });

export const saveConfig = (config: AppConfig): Effect.Effect<AppConfig, Error> =>
  Effect.gen(function* () {
    const path = yield* resolveDataFilePath(FILE_NAME);
    yield* Effect.tryPromise({
      try: async () => writeFile(path, JSON.stringify(config, null, 2), "utf8"),
      catch: (e) => (e instanceof Error ? e : new Error("Failed to write config")),
    });
    return config;
  });
