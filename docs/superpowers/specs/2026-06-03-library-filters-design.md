...
- **Refresh-Button (Nutzer-Feedback 2026-06-04):** „Aktualisieren"-Button (RefreshCw) im Kopf der Heruntergeladen-View → neuer IPC-Kanal `library:refresh` → Main führt `reloadDownloadedEntries()` aus (Einträge ohne `video.mp4` verschwinden; extern gelöschte Songs gelten danach wieder als „nicht vorhanden", auch im Such-Dedupe).
